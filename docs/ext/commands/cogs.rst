...
There comes a point in your bot's development when you want to organize a collection of commands, event handlers, and some state into one class. Cogs allow you to do just that.
...
- Every event handler is marked with the :meth:`.commands.Cog.event` decorator.
...
- All event handler must be explicitly marked via decorator, :meth:`~.commands.Cog.event`.
...
To do the same with event handler, we can query them with :meth:`.Cog.get_event_handlers`. This returns a list of tuples -- the first element being the listener name and the second one being the actual function itself. ::
