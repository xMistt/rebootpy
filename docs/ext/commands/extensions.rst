...
An extension at its core is a python file with an entry point called ``extension_setup``. This setup must be a plain Python function (not a coroutine). It takes a single parameter -- the :class:`~.commands.Bot` that loads the extension.
...
Although rare, sometimes an extension needs to clean-up or know when it's being unloaded. For cases like these, there is another entry point named ``cog_teardown`` which is similar to ``extension_setup`` except called when the extension is unloaded.
